...
*/
Design::Design(Inputs* inputs_, bool full) : inputs(inputs_){
    add_scaffold();
    add_helices();
    add_staples();
    add_domains();
    ...
    if (full){
        for (auto st = staples.begin(); st!=staples.end(); ++st){
            for (auto it = st->state.begin(); it != st->state.end(); ++it){
                (*it) = 1;
            }
        }
    }
...
void Design::add_crossovers(){
	...
	for (vector<Crossover>::iterator cross = crossovers.begin(); cross!=crossovers.end(); ++cross){
		for (int h1=1; h1<13; h1++){
			...
			else if (cross->is_seam) {cross->type = 'm';} //Leaving seams with no type.
			else if (cross->is_long && !cross->is_seam) {cross->type = 'l';} //Leaving long non-seam crossovers with no type.
			else if (cross->is_long && cross->is_seam) {cross->type = 'l';}
            else {}
		}	
	}
...
void Design::initialise_states(){
	for (auto it = staples.begin(); it!= staples.end(); ++it){
		for (auto dom = it->domains.begin(); dom != it->domains.end(); ++dom){
            ...
            //Test.push_back( (it->state[dom-it->domains.begin()]) )
		}
	}
	/*
	for (auto it = staples.begin(); it!= staples.end(); ++it){
...
//Write-Read
void Design::write_state(){
    ofstream trajectory_file;
    trajectory_file.open(inputs->trajectory_file_name, std::ofstream::out | std::ofstream::app);
    trajectory_file << "{\n";
    for (auto st = staples.begin(); st!= staples.end(); ++st){
        trajectory_file << "[";
        for (auto s= st->state.begin(); s!= st->state.end(); ++s){
            trajectory_file << (*s) << ",";
        }
        trajectory_file << "]\t";
    }
